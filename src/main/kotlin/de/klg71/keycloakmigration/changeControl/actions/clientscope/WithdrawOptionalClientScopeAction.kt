...
class WithdrawOptionalClientScopeAction(
    realm: String?,
    ...
    override fun undo() {
        val clientUUID = client.clientUUID(clientId, realm())
        val clientScopeUUID = client.clientScopeUUID(clientScopeName, realm())
        client.assignOptionalClientScope(
            realm(), clientUUID,
            clientScopeUUID, AssignClientScope(clientUUID, clientScopeUUID, realm())
        )
    }

    override fun name() = "WithdrawOptionalClientScope $clientScopeName to client: $clientId"
